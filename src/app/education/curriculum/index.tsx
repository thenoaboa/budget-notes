...
export default function CurriculumScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function loadProgress() {
        const savedLessonIds = await getCompletedLessons();

        if (screenIsActive) {
          setCompletedLessonIds(savedLessonIds);
        }
      }
  ...
  const completedLessonCount = lessons.filter((lesson) =>
    completedLessonIds.includes(lesson.id as EducationLessonId),
  ).length;

  const progressPercent =
    lessons.length === 0 ? 0 : (completedLessonCount / lessons.length) * 100;
          ...
          <View style={styles.lessonList}>
            {lessons.map((lesson, index) => {
              ...
              const completed = completedLessonIds.includes(
                lesson.id as EducationLessonId,
              );
                ...
                >
                  ...
                  {completed ? (
                    <View style={styles.completedIcon}>
                      <Ionicons name="checkmark" size={17} color="#101820" />
                    </View>
                  ) : (
                    <Ionicons
                      name={
                        available ? "chevron-forward" : "lock-closed-outline"
                      }
                      size={21}
